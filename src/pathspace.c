...
float path_shift(path_t *shifted, float pixel_i, float pixel_j, path_t *source) {
  ...
  *shifted = *source;
  ...
  volatile float J = shifted_G / source_G; // 1/source_G / 1/shifted_G
  int v = 1;
  ...
  if(v < shifted->length-1) {
    J *= (mf(shader_pdf(source, v-1), 0) * path_G(source, v)) / (mf(shader_pdf(shifted, v-1), 0) * path_G(shifted, v));
  }
